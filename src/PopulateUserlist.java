...
public class PopulateUserlist {
	...
	public static void main(String[] args) {
		...
		String role = "TechnicalLead";
		//less than
		int experience = 7;
        streamFilter(getUserMap(), role, experience).forEach(System.out::println);
		}
    ...
    return userMap;
        ...
        private static List<Customer> streamFilter(Map<Integer, Customer> userMap, String role, int exp) {

		// Role based
		Predicate<Customer> roleBased = u -> u.getRole().equalsIgnoreCase(role);
		// Experience Based
		Predicate<Customer> expBased = u -> u.getExperience() < exp;

		List<Customer> userList = userMap.values().stream().filter(roleBased).filter(expBased)
